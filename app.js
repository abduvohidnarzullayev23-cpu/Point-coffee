...
function showSection(name, btn){
  ...
  if(name==='ijara'){ loadIjaralar(); renderIjara(); }
...
function renderChart(){
  ...
  bars.innerHTML = cats.map(cat=>{
    ...
    return `<div class="chart-bar-group" title="${cat}">
      <div class="cb done" style="height:${dH}px"></div>
      ${lH ? `<div class="cb late" style="height:${lH}px"></div>` : ''}
    </div>`;
  }).join('');
}
...
function setFilter(f, btn){
  taskFilter = f;
  document.querySelectorAll('#sec-tasks .fbtn').forEach(b=>b.classList.remove('active'));
...
function renderDocuments(){
  ...
  const jd = document.getElementById('j-docs');
  if(jd) jd.textContent = documents.length;
